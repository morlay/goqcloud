...
type CreateDbInstanceHourRequest struct {
	...
	// 实例ID，购买只读实例或者灾备实例时必填，该字段表示只读实例或者灾备实例的主实例ID，请使用查询实例列表接口查询云数据库实例ID
...
func (req *CreateDbInstanceHourRequest) Invoke(client github_com_morlay_goqcloud.Client) (*CreateDbInstanceHourResponse, error) {
	resp := &CreateDbInstanceHourResponse{}
	err := client.Request("cdb", "CreateDBInstanceHour", "2017-03-20").Do(req, resp)
	return resp, err
}

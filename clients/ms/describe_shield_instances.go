package ms

import (
	github_com_morlay_goqcloud "github.com/morlay/goqcloud"
)

// 用户查询提交过的app列表
// https://cloud.tencent.com/document/api/283/17751

type DescribeShieldInstancesRequest struct {
	// 支持通过app名称，app包名，加固的服务版本，提交的渠道进行筛选。
	Filters []*Filter `name:"Filters,omitempty"`
	// 可以提供ItemId数组来查询一个或者多个结果。注意不可以同时指定ItemIds和Filters。
	ItemIds []*string `name:"ItemIds,omitempty"`
	// 数量限制，默认为20，最大值为100。
	Limit *int64 `name:"Limit,omitempty"`
	// 偏移量，默认为0。
	Offset *int64 `name:"Offset,omitempty"`
	// 升序（asc）还是降序（desc），默认：desc。
	OrderDirection *string `name:"OrderDirection,omitempty"`
	// 按某个字段排序，目前仅支持TaskTime排序。
	OrderField *string `name:"OrderField,omitempty"`
	// 区域
	Region string `name:"Region"`
}

func (req *DescribeShieldInstancesRequest) Invoke(client github_com_morlay_goqcloud.Client) (*DescribeShieldInstancesResponse, error) {
	resp := &DescribeShieldInstancesResponse{}
	err := client.Request("ms", "DescribeShieldInstances", "2018-04-08").Do(req, resp)
	return resp, err
}

type DescribeShieldInstancesResponse struct {
	github_com_morlay_goqcloud.TencentCloudBaseResponse
	// 一个关于app详细信息的结构体，主要包括app的基本信息和加固信息。
	AppSet []*AppSetInfo `json:"AppSet"`
	// 符合要求的app数量
	TotalCount int64 `json:"TotalCount"`
}
